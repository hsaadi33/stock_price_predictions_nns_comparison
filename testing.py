...
for stock in stocks:
	...
	results_lstm[stock] = LSTM.lstm(adj_close[stock],time_step,300, start_training_date,end_training_date,start_test_date,end_test_date)
	results_tcn[stock] = TCN.tcn(adj_close[stock],time_step, start_training_date,end_training_date,start_test_date,end_test_date)
	

with open('lstm_results_300_pred40_days.csv','w', newline='') as file:
...
with open('tcn_results_10_timestep_pred40_days.csv','w', newline='') as file:
	writer = csv.writer(file)
	writer.writerow(["Stocks", "RMSE train", "RMSE test", "MAE train" , "MAE test"])
	for stock in stocks:
